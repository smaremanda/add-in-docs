...
After an add-in is activated, you can use the [mailbox.item](../../../reference/outlook/Office.context.mailbox.item.html) property to obtain the currently selected item in Outlook, and the [item.itemType](http://dev.outlook.com/reference/add-ins/Office.context.mailbox.item.md) property to obtain the type of the current item.
